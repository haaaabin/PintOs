 ...
 * pointer is returned. */
/* pml4e에 가상 주소 VADDR에 대한 페이지 테이블 항목의 주소를 반환합니다.
 * PML4E에 VADDR에 대한 페이지 테이블이 없는 경우 동작은 CREATE에 따라 달라집니다.
 * CREATE가 true인 경우 새 페이지 테이블이 생성되고 그것을 가리키는 포인터가 반환됩니다.
 ...
 * allocation fails. */
/* 새 페이지 맵 레벨 4 (pml4)을 만들어 커널 가상 주소에 대한 매핑을 가지지만 사용자 가상 주소에 대한 매핑은 없습니다.
 * 메모리 할당에 실패하면 새 페이지 디렉토리를 반환하거나 null 포인터를 반환합니다. */
uint64_t *
pml4_create (void) {
...
/* 커널을 포함한 각 사용 가능한 pte 항목에 FUNC를 적용합니다. */
bool
...
/* pml4e를 파괴하고 참조하는 모든 페이지를 해제합니다. */
void
pml4_destroy (uint64_t *pml4) {
	...
	/* PML4 (vaddr) >= 1이면 define에 의해 커널 공간입니다. */
 ...
 * register. */
/* 페이지 디렉토리 PD를 CPU의 페이지 디렉토리 베이스 레지스터로 로드합니다. */
void
 ...
 * 해당 물리 주소에 해당하는 커널 가상 주소를 반환하거나 UADDR이 매핑되지 않은 경우 null 포인터를 반환합니다. */
void *
 ...
 * UPAGE는 이미 매핑되어 있으면 안 됩니다. KPAGE는 아마도 palloc_get_page()로 사용자 풀에서 얻은 페이지여야 합니다.
 * WRITABLE이 true인 경우 새 페이지는 읽기/쓰기이고, 그렇지 않으면 읽기 전용입니다.
...
/* 사용자 가상 페이지 UPAGE를 페이지 디렉토리 PD에서 "없음"으로 표시합니다.
 * 나중에 페이지에 대한 액세스는 오류가 발생합니다. 페이지 테이블 항목의 다른 비트는 보존됩니다.
...
/* PML4의 가상 페이지 VPAGE에 대한 PTE가 더티인 경우 true를 반환합니다.
 * 즉, PTE가 설치된 이후에 페이지가 수정되었는지 여부입니다.
 ...
 * in PML4. */
/* PML4의 가상 페이지 VPAGE에 대한 PTE의 더티 비트를 DIRTY로 설정합니다. */
void
...
/* PML4의 가상 페이지 VPAGE에 대한 PTE가 최근에 액세스되었는지, 즉 PTE가 설치된 시간과 마지막으로 지워진 시간 사이에 액세스되었는지 여부를 반환합니다.
 * PML4에 VPAGE에 대한 PTE가 없으면 false를 반환합니다. */
...
/* Sets the accessed bit to ACCESSED in the PTE for virtual page
   VPAGE in PD. */
/* PD의 가상 페이지 VPAGE에 대한 PTE의 액세스된 비트를 ACCESSED로 설정합니다. */
void
